...
<script>
	export default{
		...
		onLoad(e){
			...
			if (this.aid != ''){
				uni.request({
					url: 'https://bytescloud.cn/api/answer',
					method: 'GET',
					data: {
						aid:this.aid
					},
					...
					success: res => {
						...
						this.index = this.answer.atype
						this.meaning = this.answer.meaning
						this.fromWhat = this.answer.fromWhat
						this.example = this.answer.example
					},
					fail: () => {},
					complete: () => {}
				});
